...
app.post('/', async (c) => {
   const user = c.get('user')
   const { system_id, area_id, building_id, floor_id, room_id, alias_name, notification_threshold } = await c.req.json()
   ...
   try {
       await c.env.DB.prepare(`
         ...
         VALUES (?, ?, ?, ?, ?, ?)
       `).bind(system_id, area_id, building_id, floor_id, room_id, alias_name).run()
   } catch(e) {
   ...
   `).bind(system_id, area_id, building_id, floor_id, room_id).first()
   ...
   try {
       await c.env.DB.prepare(`
         ...
         ON CONFLICT(user_id, room_id) DO UPDATE SET
           ...
           is_active = 1
       `).bind(user.id, room.id, alias_name, notification_threshold || -1).run()
   } catch(e) {
