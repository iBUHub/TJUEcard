...
app.post('/poll', async (c) => {
    ...
    for (const room of pending.results) {
        ...
        `).bind(agentId, now + 300, room.id, now).run()
...
app.post('/submit', async (c) => {
    ...
    await c.env.DB.prepare(`
        ...
        SET 
            ...
            lock_agent_id = NULL
        WHERE id = ?
    `).bind(now, success ? 'success' : 'failed', electricity, message, nextTime, room_id).run()
...
async function checkAndNotify(env: any, roomId: number, electric: number) {
    ...
    for (const sub of subs.results) {
        if (electric < sub.notification_threshold) {
            ...
            console.log(`[Email Alert] To: ${sub.email} | Room ID: ${roomId} | Level: ${electric} < ${sub.notification_threshold}`)
        }
    }
}
